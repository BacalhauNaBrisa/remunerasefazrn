...
2. Preencha as verbas adicionais (dias de Ajuda de Custo, tempo de serviço/ATS).
3. Escolha o **teto remuneratório** vigente e o **regime previdenciário**.
4. Informe dependentes e outras deduções para o Imposto de Renda, e marque os descontos consignados (Sindifern/Asfarn), se aplicável.
...
| Verba | Regra |
|---|---|
| **Vencimento básico** | Valor fixo por padrão (AFRE-1 a AFRE-5), conforme tabela na própria página. |
| **UPVs** (Unidade de Parcela Variável) | `quantidade de UPVs do padrão × R$ 294,64`. Zerada se o padrão for AFRE-1 e o usuário marcar "em curso de formação" (Art. 46 da LOAT-RN). |
| **Adicional de periculosidade** | `30% do vencimento básico`, sempre devido, independente das UPVs. |
...
`Remuneração Bruta = vencimento básico + UPVs + periculosidade + ajuda de custo + ATS`

### 2. Deduções (formam a Remuneração Líquida)

| Dedução | Regra |
|---|---|
| **Redutor Art. 37/CF** | Se `(vencimento básico + UPVs + periculosidade + ATS)` ultrapassar o teto remuneratório escolhido, o excedente é descontado integralmente. |
| **Desconto previdenciário oficial** | Aplicado sobre a *base previdenciária* (`vencimento básico + UPVs + ATS`, limitada ao teto escolhido), com alíquotas progressivas por faixa (11% a 18%), conforme Portaria nº 001/2026/CRH/PR. No regime **Previdência Complementar**, essa base só é tributada até a 2ª faixa (11% e 14%). |
| **Desconto previdenciário complementar** | Só existe no regime Previdência Complementar: alíquota escolhida pelo usuário (0%, 7,5%, 8,0% ou 8,5%, padrão 8,5%) aplicada sobre a parcela da base previdenciária que excede R$ 8.475,55. |
| **Imposto de Renda** | Base de cálculo = `(vencimento básico + UPVs + periculosidade + ATS, limitada ao teto) − desconto previdenciário oficial − desconto previdenciário complementar − (dependentes × R$ 189,59) − outras deduções`, nunca negativa. Sobre essa base aplica-se a tabela progressiva por faixas (0%, 7,5%, 15%, 22,5%, 27,5%). |
| **Descontos consignados em folha** | Sindifern (0,5%) e/ou Asfarn (0,4%) sobre `vencimento básico + UPVs`, se as respectivas caixas forem marcadas (nenhuma marcada por padrão). |
